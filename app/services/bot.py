...
# «остаток наличными 5 229 отдала Махабату» (Мунара 24.09) — это передача, не её наличные
_HANDED = re.compile(r"отдал|передал|вернул|получил|взял|брал|снял", re.I)


def on_hand_amount(text: str):
    """Сумма «на руках» из текста; при «51090-12700=38 390» — результат после «=».
    Если в тексте кому-то отдали/передали — не наличные на руках, пусть читает модель."""
    if _HANDED.search(text or ""):
        return None
...
def intake_photo(db: Session, site: Organization, author: User | None, data: bytes, file_unique_id: str | None,
    ...
    if kind == drafts.RECEIPT and info.get("amount"):
        # Тот же чек вторым снимком (Мунара 25.09: в личку 09:59, в группу 10:16 — другое фото,
        # хэш другой): черновик на ту же сумму ещё ждёт проверки — второй не заводим, иначе
