...
# Меню больше не списывает со склада (09.09, решение владельца).
#
# Почему: тех.карта (состав и граммовка каждого блюда) собиралась ИИ-разбором
# документов, а не из практики кухни. Повара кладут по-своему, и расчётное
# списание не сходилось с реальным расходом — все 46 минусов на складе
# (Яйцо −1021, Говядина −41, Сахар −38) наросли именно отсюда. Подгонять 112
# рецептов под факт признано нереальным.
#
# Куда идём: отпуск продуктов заведующим складом на кухню — выдача и есть
# списание, по факту, без нормы сверху. Когда накопится история «такие блюда,
# столько детей, выдано столько сырья», из неё можно будет вывести настоящую
# норму — то есть рецепт станет результатом наблюдения, а не вводной.
#
# Меню при этом остаётся рабочим экраном: план на неделю, основа для закупа.
# Рецепты (dish_ingredients) сознательно не удаляются — они и будут тем, с чем
# сравнивать накопленный факт.
#
# Исторические списания за 24.07–27.08 тоже оставлены как есть: период, когда
# склад считался неверно, — факт, а не мусор, который надо прятать. Остатки
# приводит к реальности актуализация, а не переписывание истории.
MENU_WRITEOFF_ENABLED = False
...
def auto_apply_if_pending(db: Session, org_id: int, target_date: date_type, user_id: int) -> None:
    ...
    С 09.09 выключено флагом MENU_WRITEOFF_ENABLED — см. комментарий выше.
    Вызовы со страниц Склада и Расходов оставлены на месте намеренно: механизм
    возвращается сюда же, когда появится отпуск со склада."""
