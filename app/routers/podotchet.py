...
def create_withdrawal(
    ...
    """Закрыто 17.09 — ведёт в новый вход (см. ниже). Прежнее описание:
    ...
    # Карман здесь всегда держателя кассы, выбрать нельзя. В новом входе карман
    # выбирают явно — снятие только там (17.09).
    return RedirectResponse("/new/cash/withdraw", status_code=303)
    user, redirect = _guard(request, db)
    if redirect:
        return redirect
