...
# Первая настройка здесь — держатель кассы объекта (25.08, изначально только
# для разовых услуг, app/routers/services.py). С 04.09 то же поле стало
# держателем и для «Подотчёта» (app/routers/podotchet.py, _resolve_holder) —
# один человек на бизнес вместо свободного выбора отчитывающегося на каждом
# снятии, иначе FIFO подотчёта не может точно понять, чьими деньгами оплачен
# расход, если открыто несколько бакетов сразу. Отдельная страница, а не
# довесок на «Услуги» — по прямому запросу: настройки объекта должны жить
# своим местом, не смешиваться со страницей, где их не ждут (тот же урок, что
# и с разделением форм подотчёта). Видимость — те же роли, что «Подотчёт»
