...
def handle_confirm(
    ...
    for i, name in enumerate(item_name):
        ...
        if qty_val is None or price_val is None:
            # Называем строку: распознавание добавляет мусорные строки без количества
            # («TOKCH 150», 17.09) — человек пять раз жал «Провести», не видя, какая мешает.
            return _error_response(f"Строка «{name}»: нет количества или цены. Заполните их или удалите эту строку")
